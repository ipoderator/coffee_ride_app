// Drizzle schema root. Empty for now (CR-004 is tooling only — see
// docs/changelog.md); each domain entity gets its own file here
// (schema/user.ts, schema/ride.ts, ...) re-exported from this module, added
// via the `db-migration` skill at the point a real feature needs it (starting
// CR-011, the `users` table).
//
