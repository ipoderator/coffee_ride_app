...
/**
 * `/organizer/profile` (CR-097, KI-023 remainder, `docs/design.md` §9
 * "Avatar"). Same shape as `features/participant/profile/components/
 * AvatarUploadForm` — a distinct component, not a shared one, since the two
 * cabinets' feature modules must not depend on each other's internals
 * (`.claude/rules/extensibility.md`); only rendered once an `OrganizerProfile`
 * already exists (`OrganizerProfileForm` gates on `profile !== null`), since
 * the API requires one to upload against.
 */
