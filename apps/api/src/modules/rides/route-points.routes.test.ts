...
describe('/v1/rides/:id/route-points', () => {
  beforeEach(async () => {
  ...
  describe('POST /v1/rides/:id/route-points', () => {
    it('rejects a request with no session cookie with 401', async () => {
      const app = await buildApp(testEnv);

      const response = await app.inject({
        method: 'POST',
        url: `/v1/rides/${randomUUID()}/route-points`,
        headers: { origin: WEB_ORIGIN },
        payload: VALID_ROUTE_POINT,
      });
    ...
    it("rejects another organizer's ride with 404 ride_not_found", async () => {
      ...
      const response = await app.inject({
        method: 'POST',
        url: `/v1/rides/${owner.rideId}/route-points`,
        headers: { origin: WEB_ORIGIN },
    ...
    it('rejects an invalid payload with 400 validation_error', async () => {
      ...
      const response = await app.inject({
        ...
        headers: { origin: WEB_ORIGIN },
        cookies: { session: rawToken },
        payload: { type: 'volcano', lat: 200, lng: 37.6 },
      });
    ...
    it('creates a route point and a second of the same type, both visible on GET /:id', async () => {
      const app = await buildApp(testEnv);
      const { rawToken, rideId } = await registerAndLogin(app);
      ...
      expect(first.json().routePoint).toMatchObject({
        rideId,
        type: 'water',
        label: VALID_ROUTE_POINT.label,
        lat: VALID_ROUTE_POINT.lat,
        lng: VALID_ROUTE_POINT.lng,
        description: null,
      });
      ...
      const second = await app.inject({
        ...
        headers: { origin: WEB_ORIGIN },
        cookies: { session: rawToken },
        payload: { type: 'water', lat: 55.76, lng: 37.62 },
      });
      expect(second.statusCode).toBe(201);
      expect(second.json().routePoint.label).toBeNull();
      ...
      expect(
        detail.json().routePoints.map((p: { type: string }) => p.type),
      ).toEqual(['water', 'water']);
  ...
  describe('PATCH /v1/rides/:id/route-points/:routePointId', () => {
    async function createRoutePoint(
      ...
      return created.json().routePoint.id as string;
    }

    it('rejects a request with no session cookie with 401', async () => {
      const app = await buildApp(testEnv);
      const { rawToken, rideId } = await registerAndLogin(app);
      const routePointId = await createRoutePoint(app, rawToken, rideId);

      const response = await app.inject({
        method: 'PATCH',
        url: `/v1/rides/${rideId}/route-points/${routePointId}`,
        headers: { origin: WEB_ORIGIN },
        payload: { type: 'danger' },
      });
    ...
    it('returns 404 route_point_not_found for a non-existent route point id', async () => {
      const app = await buildApp(testEnv);
      const { rawToken, rideId } = await registerAndLogin(app);

      const response = await app.inject({
        method: 'PATCH',
        url: `/v1/rides/${rideId}/route-points/${randomUUID()}`,
        headers: { origin: WEB_ORIGIN },
        cookies: { session: rawToken },
        payload: { type: 'danger' },
      });
      ...
      expect(response.statusCode).toBe(404);
      expect(response.json().code).toBe('route_point_not_found');
      await app.close();
    });

    it("returns 404 route_point_not_found for another ride's route point", async () => {
      const app = await buildApp(testEnv);
      const owner = await registerAndLogin(app);
      const other = await registerAndLogin(app);
      const routePointId = await createRoutePoint(
        app,
        owner.rawToken,
        owner.rideId,
      );
      ...
      const response = await app.inject({
        method: 'PATCH',
        url: `/v1/rides/${other.rideId}/route-points/${routePointId}`,
        headers: { origin: WEB_ORIGIN },
    ...
    it('updates the given fields only', async () => {
      ...
      const response = await app.inject({
        ...
        headers: { origin: WEB_ORIGIN },
        cookies: { session: rawToken },
        payload: { type: 'danger', label: null },
      });

      expect(response.statusCode).toBe(200);
      expect(response.json().routePoint).toMatchObject({
        type: 'danger',
        label: null,
        lat: VALID_ROUTE_POINT.lat,
        lng: VALID_ROUTE_POINT.lng,
  ...
  describe('DELETE /v1/rides/:id/route-points/:routePointId', () => {
    async function createRoutePoint(
      app: Awaited<ReturnType<typeof buildApp>>,
      rawToken: string,
      rideId: string,
    ...
    it('deletes the route point, removing it from the ride detail response', async () => {
      const app = await buildApp(testEnv);
      const { rawToken, rideId } = await registerAndLogin(app);
      const routePointId = await createRoutePoint(app, rawToken, rideId);
      ...
      expect(detail.json().routePoints).toEqual([]);
