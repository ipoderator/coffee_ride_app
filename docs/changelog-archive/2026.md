...
Entries moved out of `docs/changelog.md` per its own "Archiving" section, once
the live file exceeded ~40 entries (CR-021 session, 2026-09-15). Content/order
preserved exactly — this file is append-only from the archiving side too (never
edited once written, only ever added to when the next archiving pass runs).
